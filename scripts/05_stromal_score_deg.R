# 05_stromal_score_deg.R
# 按 StromalScore 中位数分组，DESeq2 差异分析。
# 高 StromalScore vs 低 StromalScore，识别基质评分相关的差异表达基因。
...
# ── 输出目录 ──────────────────────────────────────────────────────────────────
out_dir <- file.path(processed_dir, "05_stromal_score_deg")
...
res <- results(dds)
save(res, file = file.path(out_dir, "DEG_StromalScore.Rda"))
...
write.csv(DEG[up_genes, , drop = FALSE],   file.path(out_dir, "Stromal_up.csv"))
write.csv(DEG[down_genes, , drop = FALSE], file.path(out_dir, "Stromal_down.csv"))
write.csv(DEG[up_genes, , drop = FALSE],   file.path(table_dir, "DEG_StromalScore_up.csv"))
write.csv(DEG[down_genes, , drop = FALSE], file.path(table_dir, "DEG_StromalScore_down.csv"))
...
colnames(annotation_col) <- "StromalScore"

exp_diff <- exp_diff[, c(rownames(subset(annotation_col, StromalScore == "high")),
                          rownames(subset(annotation_col, StromalScore == "low"))),
                     drop = FALSE]
...
pdf(file.path(figures_dir, "heatmap_StromalScore_DEG.pdf"), width = 10, height = 8)
pheatmap(exp_diff,
...
cat("StromalScore DEG done. Output:", out_dir, "\n")
